...
export function listToBlocks(
  child: List,
  option?: Option,
): Iterable<BlockObjectRequestWithoutChildren> {
  if (child.ordered) {
    return child.children.map((child) =>
      listItemToNumberedListItem(child, option),
    );
  }
  return child.children.map((child) =>
    listItemToBulletedListItem(child, option),
  );
}

function listItemToNumberedListItem(
  child: ListItem,
  option?: Option,
): NumberedListItemObjectRequest {
...
function listItemToBulletedListItem(
  child: ListItem,
  option?: Option,
): BulletedListItemObjectRequest {
